...
// --- Client Global Data ---
// HANDLE hServerRequestPipe = INVALID_HANDLE_VALUE; // << MODIFICATION: REMOVE THIS GLOBAL
...
bool send_request_and_get_response(RequestMessage& request, ResponseMessage& response) {
    ...
    // MODIFICATION: Connect to server's main request pipe for THIS transaction
    HANDLE hCurrentServerRequestPipe = CreateFile(
        SERVER_REQUEST_PIPE_NAME,
        GENERIC_WRITE, // Client only needs to write requests to this pipe
        0,
        NULL,
        OPEN_EXISTING,
        0,
    ...
    // MODIFICATION: CORRECT ORDER OF OPERATIONS
    // 1. Send the request to the server first.
    DWORD bytesWritten;
    if (!WriteFile(hCurrentServerRequestPipe, &request, sizeof(RequestMessage), &bytesWritten, NULL) || bytesWritten != sizeof(RequestMessage)) {
        ...
        CloseHandle(hCurrentServerRequestPipe); // Close the handle before returning
        return false;
    }
    // std::cout << "Client DBG: Request sent to server. Waiting for server to connect to my response pipe: " << g_my_response_pipe_name << std::endl;


    // 2. Now, wait for the server to connect to our response pipe.
    //    The server, after receiving the request, will use CreateFile with g_my_response_pipe_name.
    ...
    if (!bClientPipeConnected) {
        ...
        CloseHandle(hCurrentServerRequestPipe); // Close the handle to server's main pipe
        return false;
    }
    // std::cout << "Client DBG: Server connected to my response pipe." << std::endl;


    // 3. Read the server's response from our pipe.
    DWORD bytesRead;
    ...
    // std::cout << "Client DBG: Response received from server." << std::endl;


    // 4. Disconnect the server from this instance of our response pipe.
    DisconnectNamedPipe(hMyResponsePipe);

    // 5. MODIFICATION: Close the connection to server's main request pipe for THIS transaction
    CloseHandle(hCurrentServerRequestPipe);
...
int main() {
    ...
    // MODIFICATION: No initial persistent connection to hServerRequestPipe here.
    // It will be done per-request in send_request_and_get_response.
    // A one-time check to see if server pipe exists at all (optional).
    ...
    CloseHandle(hTestPipe); // Just a test, close it immediately.
    std::cout << "Client: Server pipe seems available. Proceeding to menu." << std::endl;
    ...
    while (running) {
        ...
        // Clear request struct for safety, though most fields will be overwritten.
        ...
        case 1: {
            ...
            if (send_request_and_get_response(request, response)) {
                if (response.type == DATA_RECORD) {
                    ...
                    // std::cin.get(); // This can be tricky with previous std::cin.ignore
                    // A more robust way to wait for just Enter:
                    std::string dummy;
        ...
        case 3: {
            ...
            if (send_request_and_get_response(request, exitResponse)) { // This now does a full connect/disconnect
